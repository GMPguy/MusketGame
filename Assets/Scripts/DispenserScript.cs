...
public class DispenserScript : GrabPoint {
    ...
    void Update(){
        if(Changed && GrabStatus == 1){
            ...
            if(HandleID == -1) newItem.GetComponent<GrabPoint>().Grab(Hand, HandIndex*2+GP);
            else newItem.transform.GetChild(HandleID).GetComponent<GrabPoint>().Grab(Hand, HandIndex*2+GP);
